...
export const removePropertyImg = async (req, res) => {
  ...
  try {
    ...
    // check
    if (req.userId !== foundProperty.creatorId.toString()) {
      return res
        .status(StatusCodes.FORBIDDEN)
        .json({ success: false, msg: "You con only update your property" });
    }
...
export const addPropertyImg = async (req, res) => {
  const { propertyId } = req.params;
...
export const getPropertyCoords = async (req, res) => {
  const { propertyId } = req.params;
  try {
...
export const updatePropertyCoords = async (req, res) => {
